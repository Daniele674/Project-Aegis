...
// Import per i selettori di data
...
const Logs = () => {
  ...
  const fetchLogs = async (filter, dates) => {
    if (!org) return;
    setLoading(true);
    ...
    if (filter.type === 'timeRange' && dates.start && dates.end) {
        endpoint = '/query/TimeRange';
        payload = {
            startUnix: Math.floor(new Date(dates.start).setHours(0, 0, 0, 0) / 1000),
            endUnix: Math.floor(new Date(dates.end).setHours(23, 59, 59, 999) / 1000)
        };
    } else {
        switch(filter.type) {
          case 'severity':
    ...
    try {
      const response = await axios.post(`http://localhost:3001${endpoint}`, payload, { headers: { 'x-org': org } });
  ...
  useEffect(() => {
    if (activeFilter.type === 'timeRange' && (!dateRange.start || !dateRange.end)) {
        return;
    }
    fetchLogs(activeFilter, dateRange);
  }, [org, activeFilter]);
  ...
  const handleApplyDateFilter = () => {
      if (dateRange.start && dateRange.end) {
          ...
          setActiveFilter({ type: 'timeRange' });
      } else {
    ...
    <LocalizationProvider dateAdapter={AdapterDateFns}>
        <Box m="20px">
          ...
          <Box mb={2} display="flex" flexDirection="column" gap={2}>
            ...
            <Box display="flex" gap={2} alignItems="center" flexWrap="wrap">
                <Typography color={colors.grey[300]}>Date Range:</Typography>
                <DatePicker
                    label="Start Date"
                ...
                <Button 
                    ...
                    variant={activeFilter.type === 'timeRange' ? 'contained' : 'outlined'}
          ...
          <Dialog open={isHistoryOpen} onClose={() => setIsHistoryOpen(false)} fullWidth maxWidth="lg">
            <DialogTitle sx={{ backgroundColor: colors.blueAccent[700] }}>History for Log ID: {selectedLogId}</DialogTitle>
